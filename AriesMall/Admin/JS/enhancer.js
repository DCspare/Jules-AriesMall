...
export async function initEnhancer() {
  ...
  // Uses a different model for Enhancer (e.g., CodeFormer) 
  const ENHANCER_MODEL_VERSION = CONFIG.ENHANCER_MODEL; 
  ...
  const enhancerContainer = document.getElementById("enhancer-container");
  if (!enhancerContainer) return;
  ...
  const urlInput = document.getElementById("enhancer-url-input");
  const addUrlBtn = document.getElementById("enhancer-add-url-btn");
  const stagingArea = document.getElementById("enhancer-staging-area");
  const controlsContainer = document.getElementById("enhancer-controls-container");
  const startBtn = document.getElementById("enhancer-start-btn");
  const progressContainer = document.getElementById("enhancer-progress-container");
  const resultContainer = document.getElementById("enhancer-result-container");
  ...
  startBtn.addEventListener("click", startBatchEnhancement);
  ...
  function stageFile(source) {
    const uniqueId = `enhancer-pending-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  function startBatchEnhancement() {
    ...
    if (!REPLICATE_API_TOKEN || !CLOUD_NAME || !UPLOAD_PRESET) {
      showToast({ title: 'Configuration Error', description: 'API tokens or Cloudinary details are not set in enhancer.js', type: 'error' });
      return;
    }
  ...
  async function processSingleFile(id, source) {
    ...
    try {
      ...
      // NOTE: statusUrl already contains the CORS_PROXY from startReplicateJob's response
  ...
  async function startReplicateJob(imageUrl, id) {
    updateProgress(id, "2/3: Starting AI enhancement...");
    
    // We expect the proxy URL here, but Replicate's API URL already contains it
    const response = await fetch(REPLICATE_API_URL, {
      method: "POST",
      headers: { Authorization: `Token ${REPLICATE_API_TOKEN}`, "Content-Type": "application/json" },
      // PARAMETERS UPDATED FOR SHARPNESS ONLY
      body: JSON.stringify({ 
        version: ENHANCER_MODEL_VERSION, 
        input: { 
          image: imageUrl,
          upscale: 1,      // 1 = No resize (keep original dimensions)
          fidelity: 0.7,   // Balance between restoration and original details (0.0 to 1.0)
  ...
  function pollForResults(statusUrl, id, name, originalUrl) {
    updateProgress(id, "3/3: AI is enhancing...");
    ...
    const poll = setInterval(async () => {
      try {
        ...
        if (result.status === "succeeded") {
          ...
          // FIX: Robustly extract the URL. CodeFormer returns an array of URLs? Actually Codeformer usually returns a single image, 
          // but depending on version it might differ. Replicate standard is 'output'.
          const enhancedUrl = result.output;
            
          if (!enhancedUrl) {
              throw new Error("AI succeeded but did not return a valid image URL in the output field.");
          }

          saveToHistory({ name: `${name} (Enhanced)`, url: enhancedUrl, type: 'enhancement' });
          displayResult(name, originalUrl, enhancedUrl);
          showToast({ title: "Enhancement Successful", description: name, type: 'success' });
  ...
  function displayResult(name, originalUrl, enhancedUrl) {
    const resultCard = document.createElement("div");
    resultCard.className = "upscaler-result-card"; // Re-using upscaler styles
    resultCard.innerHTML = `
        ...
        <div class="upscaler-result-card__body">
            ...
            <div class="upscaler-result-card__image-container">
                <span class="upscaler-result-card__label">Enhanced</span>
        ...
        <div class="upscaler-result-card__actions">
            ...
            <a href="${enhancedUrl}" download="${name.split('.')[0]}_enhanced.png" class="upload-area__button"><i data-feather="download"></i> Download</a>
        </div>`;
    ...
    resultCard.querySelector('.copy-link-btn').addEventListener('click', (e) => {
      ...
      showToast({ title: "Copied!", description: "Enhanced URL copied to clipboard.", type: 'success' });
    });
